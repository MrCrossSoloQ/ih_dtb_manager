...
async def games_search(game_date: str = None, league_id_value: int = None, nationality: str = None):
    """Funkce, která vrátí z DTB výsledky zápasů, na základě zadaného data popřípapdě ID ligy a národnosti"""
    ...
    list_of_players_stats = get_players_stats(returned_games_stats, my_dtb_controller, nationality)

    return list_of_players_stats

def get_players_stats(returned_games_stats, my_dtb_controller, nationality = None):
    for game in returned_games_stats:
        returned_game_id = game["game_id"]
        home_team_id = game["home_team_id"]
        away_team_id = game["away_team_id"]
        dtb_returned_players_stats = my_dtb_controller.get_player_game_stats(returned_game_id, nationality)
