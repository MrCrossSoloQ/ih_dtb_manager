...
class AhlGameDownloader(PlaywrightController):
    ...
    def ahl_game_manager(self):
        while True:
            ...
            returned_html_content = self.get_page_content(game_url)
            ...
            if availability_result is True:
                ...
                print(new_game)
                self.ih_games.append(new_game)
                self.last_game_url = game_url

            elif availability_result is False and self.ih_games:
                print(self.ih_games[-1].web_game_id)
                last_game_url = "https://lscluster.hockeytech.com/game_reports/official-game-report.php?client_code=ahl&game_id=" + str(self.ih_games[-1].web_game_id) + "&lang_id=1"
                self.my_dtb_driver.update_data("leagues", "schedule_url_source", last_game_url, "league_id", 2)
                print(self.ih_games)
                return self.ih_games

            elif availability_result is False and not self.ih_games:
                return False
    ...
    def game_extractor(self, game_soup, game_id):
        ...
        winner_team_id = self.get_winner_team(home_team_scraped_goals, dtb_home_team, away_team_scraped_goals, dtb_away_team)
        ...
        new_game = ih_games.IhGames(home_team_id, away_team_id, home_team_scraped_goals, away_team_scraped_goals, game_result_type, 2, winner_team_id, game_date_dtb_format, season, season_stage, game_id, None, None)
        return new_game

    def get_winner_team(self, home_team_goals, dtb_home_team, away_team_goals, dtb_away_team):
        if home_team_goals > away_team_goals:
            return dtb_home_team["team_id"]
        else:
            return dtb_away_team["team_id"]

    def get_dtb_team(self, scraped_team_name):
        dtb_team_name_list = [team["team_name"] for team in self.dtb_teams]
        name_match = get_close_matches(scraped_team_name, dtb_team_name_list, 1, 0.5)
        for dtb_team in self.dtb_teams:
            if dtb_team["team_name"] == name_match[0]:
                print(dtb_team)
                return dtb_team
    ...
    def game_date_dtb_formatter(self, game_date):
        ...
        game_date_values_in_list[1] = months_list[game_date_values_in_list[1]]
